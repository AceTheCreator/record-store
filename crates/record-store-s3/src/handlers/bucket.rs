...
use crate::xml::{
    CorsConfigurationDocument, CorsConfigurationResult, ObjectLockConfigurationDocument,
    ObjectLockConfigurationResult, VersioningConfigurationDocument,
};
use crate::*;
...
pub(crate) async fn create_bucket(
    ...
    let object_lock_enabled = match headers
        .get(OBJECT_LOCK_ENABLED_HEADER)
        .map(|value| value.to_str().unwrap_or_default())
    {
        None => false,
        Some(value) if value.eq_ignore_ascii_case("true") => true,
        // The header exists to turn the feature on. A value that is not "true"
        // is a client asking for something this adapter does not model, and it
    ...
    state
        .services
        .buckets
        .create_locked(name, None, object_lock_enabled)
        .await
...
/// Requests Object Lock for the bucket's whole lifetime, at creation only.
pub(crate) const OBJECT_LOCK_ENABLED_HEADER: &str = "x-amz-bucket-object-lock-enabled";

pub(crate) async fn put_bucket_object_lock(
    state: S3State,
    bucket: String,
    request_id: S3RequestId,
    body: Body,
    ...
    state
        .services
        .locks
        .set_bucket_configuration(&name, configuration)
        .await
        .map_err(|error| service_error(error, request_id, &bucket))?;
    Ok(StatusCode::OK.into_response())
}
...
pub(crate) async fn get_bucket_object_lock(
    ...
    xml_response(
        StatusCode::OK,
        &ObjectLockConfigurationResult::new(
